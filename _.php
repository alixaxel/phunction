...
class phunction_Net extends phunction
{
	...
	public static function OpenID($url = null, $realm = null, $return = null, $redirect = true, $verify = false)
	{
		$data = array();

		if (($verify !== true) && (parent::Value($_REQUEST, 'openid_mode') !== false))
		{
			if (strcmp('id_res', parent::Value($_REQUEST, 'openid_mode')) === 0)
			{
				...
				if (preg_match('~is_valid\s*:\s*true~i', self::CURL(self::OpenID($_REQUEST['openid_identity'], false, false, false, true), $data, 'POST')) > 0)
				{
			...
			if (ph()->Is->URL($server) === true)
			{
				if ($redirect === true)
				{
					...
					if (preg_match('~rel="openid2[.]provider"|http://specs[.]openid[.]net/auth/2[.]0~i', $result) > 0)
					{
						...
						{
							$delegate = parent::Value(ph()->Text->Regex($result, '<(Local|Canonical)ID[^>]*>([^>]+)</\1ID>', 1), 0);
					...
					parent::Redirect(sprintf('%s%s%s', $server, (strpos($server, '?') !== false) ? '&' : '?', http_build_query($data, '', '&')));
				}
